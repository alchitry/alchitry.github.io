...
In the Artix 7 used by the [Au](https://shop.alchitry.com/products/alchitry-au) and [Pt](https://shop.alchitry.com/products/alchitry-pt), each LUT is a 6-input LUT. 
...
When it is configured as two 5-input LUTs, _In\[5]_ is set to 0.
...
The FPGA used by the Au has 5,200 slices in it for a total of 20,800 LUTs.
The Pt has just about tripled that with 15,850 slices and 63,400 LUTs!

All slices are not created equal, however. 
In the Artix 7 FPGAs there are two types of slices, _SLICEL_, and _SLICEM_. 
...
A SLICEM is the same as a SLICEL except the LUTs can be used as 64 bits of RAM or a shift register up to 32 bits long. 
The breakdown of the slices is roughly SLICEL 70%, and SLICEM 30%.
...
(on Alchitry boards this happens automatically via the on-board persistent memory).
...
An ASIC design can reach speeds faster than 4GHz, while an FPGA is very fast if it's running at 450MHz (internally, IO can go faster). 
...
If you're interested in digging into the nitty-gritty, check out the official [UG474 doc from Xilinx](https://docs.amd.com/r/en-US/ug474_7Series_CLB).